...
export class PartnerService {
  ...
  async createPartner(createDto: CreatePartnerDto) {
    const existingPartner = await this.partnerRepository.findBySlug(createDto.slug);
    if (existingPartner) {
  ...
  async updatePartner(id: string, updateDto: UpdatePartnerDto) {
    ...
    if (updateDto.slug && updateDto.slug !== partner.slug) {
      const existingPartner = await this.partnerRepository.findBySlug(updateDto.slug);
      if (existingPartner) {
        throw new BadRequestException('Partner with this slug already exists');
      }
    }
  ...
  async createPartnerHost(createDto: CreatePartnerHostDto) {
    const existingHost = await this.partnerHostRepository.findByHost(createDto.host);
    if (existingHost) {
      throw new BadRequestException('Partner host with this host already exists');
    }

    const existingSlug = await this.partnerHostRepository.findBySlug(createDto.slug);
    if (existingSlug) {
      throw new BadRequestException('Partner host with this slug already exists');
    }
    ...
    if (networks && networks.length > 0) {
      ...
      await Promise.all(
        networkEntities.map(network => this.partnerHostNetworkRepository.create(network))
      );
    }
  ...
  async regenerateAccessToken(id: string) {
    ...
    const newAccessToken = await this.partnerHostRepository.generateAccessToken();
  ...
  async getPartnerNetworks(partnerHostId: string) {
    const networks = await this.partnerHostNetworkRepository.findByPartnerHost(partnerHostId);
    return {
      success: true,
      data: {
        networks
  ...
  async addNetworkToPartnerHost(partnerHostId: string, createDto: CreatePartnerNetworkDto) {
    const partnerHost = await this.partnerHostRepository.findById(partnerHostId);
    if (!partnerHost) {
      throw new NotFoundException('Partner host not found');
    }

    const existingNetwork = await this.partnerHostNetworkRepository.findByNetwork(
      partnerHostId, 
      createDto.network as NetworkType
    );
    if (existingNetwork) {
      throw new BadRequestException('Network already exists for this partner host');
    }

    if (createDto.default) {
      await this.partnerHostNetworkRepository.setDefaultNetwork(partnerHostId, '');
    }
  ...
  async updatePartnerNetwork(partnerHostId: string, networkId: string, updateDto: UpdatePartnerNetworkDto) {
    ...
    if (updateDto.default) {
      await this.partnerHostNetworkRepository.setDefaultNetwork(partnerHostId, networkId);
    }

    const updatedNetwork = await this.partnerHostNetworkRepository.update(networkId, updateDto);
    return {
      success: true,
      data: updatedNetwork
    };

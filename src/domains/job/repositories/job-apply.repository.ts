...
export class JobApplyRepository implements IBaseRepository<JobApply> {
  ...
  async findWithFilters(filters: any): Promise<[JobApply[], number]> {
    ...
    if (filters.status) {
      queryBuilder.andWhere('jobApply.status = :status', {
        status: filters.status,
      });
    }

    if (filters.jobId) {
      queryBuilder.andWhere('jobApply.jobId = :jobId', {
        jobId: filters.jobId,
      });
    }

    if (filters.talentId) {
      queryBuilder.andWhere('jobApply.talentId = :talentId', {
        talentId: filters.talentId,
      });
    }

    if (filters.jobStatus) {
      queryBuilder.andWhere('job.status = :jobStatus', {
        jobStatus: filters.jobStatus,
      });
    }

    if (filters.jobNotStatus) {
      queryBuilder.andWhere('job.status != :jobNotStatus', {
        jobNotStatus: filters.jobNotStatus,
      });
    }
